...
public abstract class MultiTileEntityReactorCore extends TileEntityBase10FacingDouble implements ITileEntityServerTickPost, IFluidHandler, ITileEntityTapAccessible, ITileEntityFunnelAccessible, ITileEntityRunningActively, ITileEntitySwitchableOnOff, IMTE_GetCollisionBoundingBoxFromPool, IMTE_OnEntityCollidedWithBlock {
	...
	protected IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
		return FL.Coolant_IC2.is(aFluidToFill)
			...
			|| MT.Sn.mLiquid.isSameFluid(aFluidToFill)
			|| MT.Na.mLiquid.isSameFluid(aFluidToFill)
			|| MT.HDO.mLiquid.isSameFluid(aFluidToFill)
			|| MT.D2O.mLiquid.isSameFluid(aFluidToFill)
			|| MT.T2O.mLiquid.isSameFluid(aFluidToFill)
			|| MT.LiCl.mLiquid.isSameFluid(aFluidToFill)
			|| MT.He.mGas.isSameFluid(aFluidToFill)
			|| MT.CO2.mGas.isSameFluid(aFluidToFill)
			? mTanks[0] : null;
	}
	...
	public int funnelFill(byte aSide, FluidStack aFluid, boolean aDoFill) {
		if (!( FL.Coolant_IC2.is(aFluid)
			...
			|| MT.Sn.mLiquid.isSameFluid(aFluid)
			|| MT.Na.mLiquid.isSameFluid(aFluid)
			|| MT.HDO.mLiquid.isSameFluid(aFluid)
			|| MT.D2O.mLiquid.isSameFluid(aFluid)
			|| MT.T2O.mLiquid.isSameFluid(aFluid)
			|| MT.LiCl.mLiquid.isSameFluid(aFluid)
			|| MT.He.mGas.isSameFluid(aFluid)
			|| MT.CO2.mGas.isSameFluid(aFluid)
			)) return 0;
	...
	@Override public AABB getCollisionBoundingBoxFromPool() {return box(PX_P[1], PX_P[1], PX_P[1], PX_N[1], PX_N[1], PX_N[1]);}
